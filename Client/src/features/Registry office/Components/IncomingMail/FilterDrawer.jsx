  ...
  React.useEffect(() => {
    if (currentFilters) {
      setFilters({
        ...
        expediteur: currentFilters.expediteur || '',
      ...
      <LocalizationProvider dateAdapter={AdapterDayjs}>
        <Box sx={{ width: { xs: '100vw', sm: 400 }, p: 3 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            {/* Filtres actifs */}
            {(filters.dateDebut || filters.dateFin || filters.expediteur || filters.statut) && (
              <Box sx={{ mt: 2 }}>
                <Typography variant="caption" sx={{ color: '#64748B' }}>
                  Filtres actifs :
                </Typography>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 1 }}>
                  {filters.expediteur && <Chip label={`Expéditeur: ${filters.expediteur}`} size="small" />}
