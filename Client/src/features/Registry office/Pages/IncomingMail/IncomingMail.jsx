...
// StatusChip utilisant les couleurs du thème (warning, success, error, info)
const StatusChip = styled(Chip, {
  ...
  const statusConfig = {
    ...
    'Transmis PDG': { bg: alpha(theme.palette.info.main, 0.1), color: theme.palette.info.dark },
  };
...
const initialCourriers = [
  {
    ...
    version2: null,
    historique: [
      { date: '2026-05-02 09:30', action: 'Réception papier', user: 'K.M.' },
      { date: '2026-05-02 10:15', action: 'Scan V1 et saisie', user: 'K.M.' },
      { date: '2026-05-02 11:00', action: 'Transmission au PDG', user: 'K.M.' },
    ],
  ...
  },
  {
    id: 'CE-2026-002',
    date: '2026-05-02',
    expediteur: 'Direction Régionale',
    sujet: 'Convocation réunion mensuelle',
    ...
    historique: [
      { date: '2026-05-02 10:00', action: 'Réception papier', user: 'K.M.' },
      { date: '2026-05-02 10:45', action: 'Scan V1', user: 'K.M.' },
      { date: '2026-05-03 14:00', action: 'Scan V2 final', user: 'K.M.' },
    ],
  ...
  },
  {
    ...
    sujet: 'Instructions nouvelles procédures',
    assigneA: 'Mohamed Larbi',
    initiales: 'ML',
    statut: 'En attente',
    version1: './../../../../../public/pdfs/14281.pdf',
    version2: null,
    historique: [
      { date: '2026-05-03 08:00', action: 'Réception papier', user: 'K.M.' },
      { date: '2026-05-03 09:30', action: 'Scan V1', user: 'K.M.' },
    ],
    documentPhysique: 'Bureau ordre',
  },
];
...
export default function IncomingMail() {
  ...
  const [courrierToAssign, setCourrierToAssign] = React.useState(null);
  ...
  const filteredCourriers = courriers.filter((courrier) => {
    ...
    const searchMatch =
      ...
      courrier.assigneA.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleExportPDF = () => {
    exportToPDF(filteredCourriers, 'Courrier Entrant');
  ...
  const handlePrintBordereau = (courrier) => {
    ...
    doc.setFontSize(18);
    doc.text('SOTAVI ERP - Bordereau de suivi', 105, 25, { align: 'center' });
    ...
    doc.text(`Sujet: ${courrier.sujet}`, 20, 85);
    doc.text(`Assigné à: ${courrier.assigneA}`, 20, 95);
      ...
      <Dialog open={formDialogOpen} onClose={() => setFormDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            label="Assigné à"
            size="small"
            value={newCourrier.assigneA}
            onChange={e => setNewCourrier({ ...newCourrier, assigneA: e.target.value })}
            sx={{ mb: 2 }}
          />
          <TextField
            fullWidth
            ...
            <DialogContent dividers>
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, md: 6 }}>
                  ...
                  <Typography variant="body2">{selectedCourrier.assigneA}</Typography>
